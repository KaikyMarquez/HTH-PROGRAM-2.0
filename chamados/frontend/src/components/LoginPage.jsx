...
function LoginPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/auth/login', { // Usando sua rota correta
        method: 'POST',
      ...
      // SALVANDO O TOKEN E O USUÁRIO
      localStorage.setItem('authToken', data.token);
      localStorage.setItem('user', JSON.stringify(data.user)); // A linha foi adicionada aqui
